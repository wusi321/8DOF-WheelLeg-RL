...
| **惩罚坏状态是安全的**，只有"门控在坏状态上的正奖励"会被刷 | `fallen_tax` 平铺 −0.5/s，带**滞回**：摔倒后一直扣，直到真正站起来（倾角 <25° 且净空 >0.11 m）才解除 |
...
fallen_tax        -0.5    attempt scale    0.1×
```

一次完整的"趴下 → 站起"约拿到 Δcos(tilt)≈+1 ×5 = **+5**，Δ净空≈+0.14 ×30 = **+4.2**，再加上赏金 **+10**；而躺着不动是 **−0.5/s**。这就是"起立完成的奖励大于惩罚"的量化形式。
...
- `_down_seconds`：连续倒地时间，超过 `MAX_DOWN_TIME = 6 s` 回收（兜底，对应 MicroDuck 的 backstop）。
- `_fold_seconds`：**收腿完成之后**才起算，超过 `FOLD_STAND_DEADLINE = 2 s` 还没站起来就回收——这就是"收腿完成后 z 在 2 s 内没回到竖直就重置"。

分两个计时器是因为：斜着摔、方形机身卡住时**根本收不了腿**，此时只有兜底计时器生效；而收好腿却站不起来，说明这次恢复已经失败，早点回收省样本。

**新增观测**：`Metrics/fallen`、`Metrics/recovered`、`Metrics/fold_complete`、`Metrics/folded_pose_error`。

### 反向课程：一部分 episode 直接**从趴着开始**

**这是"能不能学会起身"的关键，不是可选项。** 只从站立开始的话，策略一落地就摔倒，之后 6 s 里在倒地状态乱动，**从来没有见过"站起来"这一支的成功样本**——势能增量奖励的**正向那一半永远不会被探索到**，训练就卡在"躺平静止"的平坦最优点上（每次 episode 几乎一样 → advantage 近乎为 0 → 策略完全不变）。
