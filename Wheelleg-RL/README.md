...
MicroDuck 的实测预算（`AGENTS.md`）：**简单的 episodic 技巧约 1000 轮 @4096 环境；步态与含课程的恢复需要 4000–6000 轮**。本项目 `rl_cfg.py` 的 rough 预设是 12000 轮，方向一致。

但**轮数不是唯一变量**：MicroDuck 记录了多次"跑够轮数仍然 0 次恢复"的失败，原因都在奖励设计（0/800 轮无恢复、赏金阈值够不着、尝试税让"什么都不做"胜出）。**先把结构改对，再谈轮数。**

同时它们的硬性纪律，本项目照做：

```bash
# 任何长训练之前先跑 smoke test，成本极低，能抓到约 95% 的配置错误
ITERS=5 NUM_ENVS=64 bash scripts/train.sh rough
```

**并且先验证物理再训练**：目标姿态必须是稳定平衡点（保持该 ctrl 3 s，从带噪声的初始状态出发，检查**倾角**而不只是高度——只看 z 的测试会把倒地状态报成"稳稳停住"）。`scripts/folded_pose_check.py` 就是这条纪律的产物。
...
| `body_level` | 奖励 **−10** | 机身倾离世界竖直 | 横滚始终计费；前倾 0.30 rad 内免费；后仰 1.5 倍计费；**倒地且被指令站起时缩到 0.1 倍**（见下） |
| `base_height_l2` | 奖励 −4 | 目标 **0.145 m**，高速时按速度下调 | 始终生效，低姿态轻微扣分、趴地扣分明显 |
| `standing_pose` | 奖励 −0.5 | 标准站姿 | 轻微约束腿型，不阻止抬腿 |
| `fallen_too_long` | **终止** | 倒地持续超过 **2.5 s** | 摔倒不再立即重置；窗口故意短（见下） |
...
`Episode_Reward/x` **不是**它在一整个 episode 里的贡献。`reward_manager.py:106-110`：

```python
extras["Episode_Reward/" + key] = episodic_sum_avg / self._env.max_episode_length_s
```

分母是 `max_episode_length_s`（本任务 **20 s**），不是本 episode 实际的长度。所以：

```
episode 真实贡献 = Episode_Reward/x × 20
```

验证方法：把某次日志里所有 `Episode_Reward/*` 加起来再 ×20，应该约等于同一次日志的 `Mean reward`（那一窗内结束的 episode 的回报）。第 800 轮那次实测 Σ≈−3.06 → ×20 = −61，而 `Mean reward = −68.9`，对得上。

按 ×20 折算才知道谁在主导。第 800 轮的真实收支是 `body_level −38`、`stair_lateral_yaw_drift −16`、`base_height_l2 −8`、`fallen_tax −3`，而全部正项加起来只有 `upward +8.4`、速度跟踪 +3.5。**这就是"趴着不动"成为最优点的原因**：`body_level`（当时 −10、线性倾角）是全场最大的一项，而趴平了它就接近 0；站起来的赏金（+10 一次性 + 4.2 势能）相比之下既小又稀疏。

同理，`Episode_Termination/*` 也**不是比例**（出现过 1.75），分母不是环境数，不要当占比读。`Metrics/posture/posture_alpha` 则是对**当步复位的少数环境**取的均值（`CommandTerm.reset` 只在这一刻上报），会在 0.015↔1.000 之间乱跳，那是采样假象，跨 run 比它没有意义。
...
### 摔倒与起立：照搬 MicroDuck VelStand 的成熟结构

摔倒起立的奖励设计参考 `microduck_rl/`（MicroDuck 的 VelStand 任务，注释里记录了多轮失败实验）。它的结论直接套用到本项目：

| 规则（原文教训） | 本项目的实现 |
|---|---|
| **绝不把正奖励门控在"坏状态"上** —— 策略会停在那个人为最省力的姿势刷分 | `upright_progress`、`height_progress` 都是**势能增量**（Δcos(tilt)、Δmin(净空, ceiling)）：上升给钱、保持给 0、下降扣钱，**无法刷** |
| **惩罚坏状态是安全的**，只有"门控在坏状态上的正奖励"会被刷 | `fallen_tax` 平铺 −1.0/s，带**滞回**：摔倒后一直扣，直到真正站起来（倾角 <25° 且净空 >0.11 m）才解除；被指令趴下时不扣（`standing_command` 门控） |
| **发现技能期间不能收"尝试税"** | `action_rate`、`joint_acc`、`joint_pos_limits` 在倒地时**缩到 0.1 倍**（不是关掉） |
| **完成判据必须"够得着"** | 它们第一版要求 trunk z > 0.105，实际永远到不了，赏金从未触发。本项目用**倾角 <25° 且净空 >0.11 m**（站姿 0.145 的 76%），并预留调参 |
| **一次完成给一次赏金**，重新武装必须再次摔倒 | `recovery_success`：连续倒地 ≥0.5 s 后站起来的那一帧给 +10，在阈值附近反复抖动不再给分 |
| **摔倒需要有限的处理窗口**，否则失败的恢复会占满 episode、饿死行走数据 | `fallen_too_long` 双计时器（见下） |

权重（照搬 MicroDuck 的终值，因为两台机器人的高度尺度接近——它 trunk z 约 0.117 m，本项目 0.145 m）：

```
upright_progress  +5      height_progress  +30     recovery_success  +10
fallen_tax        -1.0    attempt scale    0.1×    body_level        −10（倒地时 0.1×）
```

一次完整的"趴下 → 站起"约拿到 Δcos(tilt)≈+1 ×5 = **+5**，Δ净空≈+0.14 ×30 = **+4.2**，再加上赏金 **+10**；而躺着不动是 **−1.0/s**。这就是"起立完成的奖励大于惩罚"的量化形式。

### 收起的姿态与双计时器重置

`完全趴下落地.txt` 给的角度经 `scripts/folded_pose_check.py` 验算：

```
髋 +0.91  大腿 +1.31  膝 -2.62                    ← 报告值
髋 +0.90757  大腿 +1.308997  膝 -2.62             ← 实际采用
   = HIP_LIMIT[1], THIGH_LIMIT[1]（直接引用限位常量，不是字面量）
```

**报告的前两个值略超硬限位**（0.91 > 0.90757、1.31 > 1.30900），而且**两个都远超软限位**（`soft_joint_pos_limit_factor=0.95` → 髋 0.86219、大腿 1.24355），所以保持这个姿态会持续触发 `joint_pos_limits`（权重 −5）——这正是它必须随倒地缩放到 0.1 倍的原因。

几何验算（`uv run python scripts/folded_pose_check.py`）：

```
机身 mesh 在机身坐标系的 z 范围 = 0.0000 .. 0.0600 m
收起姿态下轮轴 z（机身坐标系）  = +0.0311 m
机身贴地时轮子最低点            = +0.0011 m   → 轮子离地 1.1 mm ✓
```

**"轮子完全收起、身体完全贴地"得到几何确认**，但余量只有 1.1 mm。

重置用**双计时器**：

- `_down_seconds`：连续倒地时间，超过 `MAX_DOWN_TIME = 2.5 s` 回收（兜底，对应 MicroDuck 的 backstop）。
- `_fold_seconds`：**收腿完成之后**才起算，超过 `FOLD_STAND_DEADLINE = 2 s` 还没站起来就回收——这就是"收腿完成后 z 在 2 s 内没回到竖直就重置"。

两个计时器都门控在 `standing_command > 0.5`：**被指令趴下的机器人不是在"没能站起来"**，它不会被回收。

**窗口故意从 6 s 缩到 2.5 s**，这是第 800 轮那次失败直接教的。斜着摔、方形机身卡住时根本收不了腿，兜底计时器是唯一生效的那个，于是每个 episode 都是 6.8 s 的"卡住 + 乱动"。那 6.8 s 里 `body_level` 一项就累积到约 **−38**（占整局 −61 的 63%，因为卡住的机身倾角约 0.5 rad，已经贴到 `MAX_TILT_COST` 上限），而策略**无法通过任何动作把那个倾角扳回来**。结果是优势函数被这一项主导，std 从 0.80 退火到 0.316，策略学会了"尽量别动"。

同一处的两个改动：

| 改动 | 数值 | 理由 |
|---|---|---|
| `body_level` 在**倒地且被指令站起**时缩放 | ×0.1 | 拿不到的梯度不该占主导。被指令**趴下**时仍是满权重——趴下必须趴平，这是硬要求（`recovery_scale` 与 `attempt_scale` 的区别就在这个门控） |
| `fallen_tax` | −0.5 → **−1.0 /s** | `body_level` 倒地时几乎不扣了，得由它来保证"被叫站起来却赖着"不划算 |

缩短窗口还顺带放大了赏金的相对价值：单次失败尝试的累积惩罚变小，而 `recovery_success` 的 +10 是**一次性**的、不随窗口缩放。另外每单位训练时间内**恢复尝试的次数**也变多了。

**注意**：窗口缩短后 episode 变短，所有 `Episode_Reward/*` 会成比例缩小（分母还是 20 s），跨版本比较时必须先 ×20 换成 episode 贡献。

**新增观测**：`Metrics/fallen`、`Metrics/recovered`、`Metrics/fold_complete`、`Metrics/folded_pose_error`。

### 反向课程：一部分 episode 直接**从趴着开始**

**这是"能不能学会起身"的关键，不是可选项。** 只从站立开始的话，策略一落地就摔倒，之后 2.5 s 里在倒地状态乱动，**从来没有见过"站起来"这一支的成功样本**——势能增量奖励的**正向那一半永远不会被探索到**，训练就卡在"躺平静止"的平坦最优点上（每次 episode 几乎一样 → advantage 近乎为 0 → 策略完全不变）。

MicroDuck 的 VelStand 记录里对这一点有明确结论，它们称之为
**"the reliable fix for 'learns the start, never the last mile'"**，并且实测"prone-init 的 episode 大部分时间花在**走到**深蹲"是另一个坑，所以它们直接**把环境重置到恢复中途的姿态**。

本项目按同一思路加了 `spawn_fallen`（`mode="reset"`）：

| 出生状态 | 概率 | 姿态 | 机身原点高度 | 出生后收到的姿态指令 |
|---|---:|---|---:|---|
| 正常站立 | **50%** | 原初始化 | 0.145 m | **站姿（α=1）** |
| **完全收起（趴下）** | **35%** | `FOLDED_STANCE`，机身平贴地面 | `FOLDED_REST_Z = 0.002 m` | 一半 **α=1**（起身任务）、一半 **α=0**（保持趴平） |
| 恢复中途的蹲姿 | 15% | `CROUCH_STANCE`（收起与站姿的中点） | `CROUCH_SPAWN_Z = 0.0761 m` | **α=1**（起身任务） |

三个高度都不是猜的：收起姿态的机身高度由 `base_link` 网格的 z 范围（0..0.06 m）算出，中点蹲姿由 `stance.clearance()` 的 FK 算出（0.0761 m）。**这些姿态常量放在 `stance.py`（纯几何、不依赖 torch），MDP 侧只是引用。**

**站姿出生占比最大是故意的。** 平衡和行走才是这台机器存在的意义；出生比例里大部分 episode 从地面开始时，其余所有奖励项所依赖的行走数据就被饿死了 —— 这正是单项倾角惩罚能主导整个收益结构的原因。实际生效的初始条件分布：**50% 站立行走**、32.5% 需要起身、17.5% 保持趴平。保持趴平的那一半专门训练"腿完全收起、机身平贴地面"这个姿态本身，趴下动作不是靠"被指令站着时摔倒"顺带学会的。

### 出生姿态决定首条姿态指令（顺序 bug）

`_reset_idx` 里 **事件管理器在最前面**（`manager_based_rl_env.py:560`），**命令管理器晚 21 行**（`:581`）。所以出生姿态和 `posture` 指令原本是**独立抽样**的：25% 的站姿出生环境里约 2/3 会抽到 α<1 的指令，动作偏移瞬间从站姿跳到趴姿，**腿被在第一帧就抽走 → 直接向前栽倒**。这不是"任务难"，这是保证摔倒，而且它教会策略"站着没有好结果"。

现在 `spawn_fallen_state` 把自己写下的姿态对应的指令值记在 `env._spawn_posture`，`PostureCommand._resample_command` 在 reset 时**消费**它（消费后置 NaN，所以 episode 中途的重采样仍然是自由抽样）。

**另外 α 现在是限速斜坡**（`transition_rate = 2.0 /s`，见 `_update_command`）。原来 α 是阶跃的，偏移量一步跳到位，等于腿被瞬移 —— 策略无法把这个和"任务不可能"区分开。限速之后"从站立趴下"才是一条能保持平衡的轨迹，而不是一次摔倒。

**这对你的动作序列正好是"从收起状态开始训练站起来"**：出生即 `fold_complete = True`，`height_progress`（权重 +30）立刻在净空从 0 → 0.145 的过程中付款（合计约 +4.2），`recovery_success` 在净空 >0.11 m 时给 +10。因为势能塑形**对部分进步也付款**，2 s 的收腿期限并不会挡住发现过程——不需要像 MicroDuck 那样把窗口放到 5–8 s（它们当时的动作第一阶段"走到深蹲"本身不产生进步，所以才需要长窗口）。

`play` 时也会按这个概率出生，所以一次 `--num-envs 4` 就能同时看到"从趴着起身"和正常行走。

### 「倒地」只有一个定义

**`FALLEN_TILT = 0.70 rad` 或 `FALLEN_CLEARANCE = 0.08 m`**（倾角或高度，**不含机身触地**——方形机身可能侧向卡住而触地传感器不触发）。这一个定义同时决定：恢复奖励的启用、起身期限的起算、以及**步态惩罚的暂停**（`upright_gate`）。

**曾经这里有两个定义**：`is_down` 用「倾角 >0.9 或机身触地」，`fallen_mask` 用「倾角 >0.7 或净空 <0.08」。结果是**"因高度而倒地但没倾斜"的机器人仍被全额收取爬行惩罚**——实测 `low_posture_locomotion` 冲到 **−6.27/s**，把上一轮已经消掉的"叠加惩罚"问题重新引了回来，平均奖励也从 −48 恶化到 −65.6。已统一。

**注意**：出生状态改造之后，`Episode_Metrics/fallen` 和 `recovered` 会被出生比例影响（现在 50% 的 episode 是**故意**从低位开始），**不能再和改造前的数值直接比较**。
...
**这条约定长期只写在文档和 `leg_symmetry_error` 里，展开姿态时却违反它。** 所有姿态展开都写成 `pose * 2`，而 `FOLDED_STANCE` 的髋是 0.90757，所以被指令的趴下姿态实际是**左髋 +0.91、右髋 +0.91**：左轮收上去、右轮伸下去，机身被顶成 55° 侧倾卡住，`fold_complete` 上不去，而且它**因为执行自己的指令反被 `leg_symmetry` 罚** 1.81² × 0.5 = 1.65。

现在 `stance.leg_joint_positions()` 是这条约定的**唯一实现**，所有展开都经过它：姿态目标（`posture.target_joint_pos`）、动作偏移（`lowpass_actions._folded_for_targets`）、`folded_pose_error` / `fold_complete`、出生姿态、`standing_pose_error`。单腿几何验算**永远查不出这个 bug**——镜像把 `y` 取反、`z` 完全不变，所以 `folded_pose_check.py` 两边都通过，问题只出现在横向摆放上。`tests/test_stance.py` 现在直接在旋转上验证这条不变式，并禁止任何地方再用 `pose * 2` 展开。
