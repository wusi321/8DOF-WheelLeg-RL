...
| 出生状态 | 概率 | 姿态 | 机身原点高度 |
|---|---:|---|---:|
| **完全收起（趴下）** | **50%** | `FOLDED_STANCE`，机身平贴地面 | `FOLDED_REST_Z = 0.002 m` |
| 恢复中途的蹲姿 | 25% | `CROUCH_STANCE`（收起与站姿的中点） | `CROUCH_SPAWN_Z = 0.0761 m` |
| 正常站立 | 25% | 原初始化 | 0.145 m |
...
**注意**：出生状态改造之后，`Episode_Metrics/fallen` 和 `recovered` 会被出生比例影响（75% 的 episode 是**故意**从低位开始），**不能再和改造前的数值直接比较**。
