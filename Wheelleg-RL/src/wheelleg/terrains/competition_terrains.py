...
class RCWallTerrainCfg(SubTerrainCfg):
    ...
    The wheelleg must repeatedly step/vault over transverse walls and proceed.
...
class RCLowBarTerrainCfg(SubTerrainCfg):
    ...
    A horizontal bar is placed along the y-axis, requiring the wheelleg to crouch and duck under.
    ...
    The actual competition spec is 0.30m闆㈠湴 and 1.0m width.
    ...
    def function(
        ...
        # the wheelleg ample room to sprint/accelerate instead of spawning directly under the bar.
        origin = np.array([1.5, cy, 0.0])
        return TerrainOutput(origin=origin, geometries=geometries)
...
class RCPyramidStairsTerrainCfg(BoxPyramidStairsTerrainCfg):
    ...
    This ensures the wheelleg can sprint/accelerate on flat ground before ascending,
