...
def base_height_l2(
    ...
    if has_sensor:
        ...
        # The wheelleg target height is ~0.36m, so the scanner should measure something between 0.1m and 0.8m usually.
...
def feet_clearance(
    ...
    if asset_cfg is None:
        asset_cfg = SceneEntityCfg("wheelleg", body_names=(".*_wheel_Link",))
    asset: Entity = env.scene[asset_cfg.name]
...
def wheel_roll_tracking(
    ...
    if asset_cfg is None:
        asset_cfg = SceneEntityCfg("wheelleg", joint_names=(".*_wheel_joint",))
    asset: Entity = env.scene[asset_cfg.name]
    command = env.command_manager.get_command(command_name)
...
def adaptive_leg_motion_penalty(
    ...
    if asset_cfg is None:
        asset_cfg = SceneEntityCfg("wheelleg", joint_names=(
            "(left|right)_hip_joint",
            "(left|right)_thigh_joint",
...
def hip_deviation(
    ...
    if asset_cfg is None:
        asset_cfg = SceneEntityCfg("wheelleg", joint_names=("(left|right)_hip_joint",))
    asset: Entity = env.scene[asset_cfg.name]
...
def joint_deviation_l2(
    ...
    if asset_cfg is None:
        asset_cfg = SceneEntityCfg("wheelleg", joint_names=("(left|right)_hip_joint", "(left|right)_thigh_joint", "(left|right)_knee_joint"))
    asset: Entity = env.scene[asset_cfg.name]
    q = asset.data.joint_pos[:, asset_cfg.joint_ids]
    q0 = asset.data.default_joint_pos[:, asset_cfg.joint_ids]
    reward = torch.sum(torch.square(q - q0), dim=1)
...
class variable_posture:
    ...
    def __call__(
        ...
        # 鎻愬彇韬綋鍊炬枩搴?(閲嶅姏鍦?XY 骞抽潰鐨勬姇褰?
        tilt_xy = torch.norm(asset.data.projected_gravity_b[:, :2], dim=1)
        
        # 濡傛灉韬綋鍊炬枩 > 0.15 (绾?8.6 搴?锛岃鏄庢鍦ㄧ埇澧欍€佽秺闅滄垨宸叉憯鍊?
        is_tilting = (tilt_xy > 0.15).float()
        is_running_cmd = (total_speed >= running_threshold).float()
        
        # 銆愭牳蹇冭秺闅滀慨鏀广€戝彧瑕侀€熷害鏋佸揩锛屾垨鑰呮鍦ㄥ€炬枩鏀€鐖紝鐩存帴璧嬩簣鏈€澶у箙搴︾殑鍔ㄤ綔瀹瑰繊鏉?(running_mask)
        running_mask = torch.clamp(is_running_cmd + is_tilting, 0.0, 1.0)
        
        # 鍓╀笅鐨勫钩绋充笖涓嶅€炬枩鐨勭姸鎬侊紝鍐嶅幓鏍规嵁浣庨€熸寚浠ゅ垽鏂槸 walking 杩樻槸 standing
...
def crouch_height_reward(
    ...
    """Reward the wheelleg for keeping body low under target height when in crouch mode (mode_id == 2).
...
def crawl_height_reward(
    ...
    """Reward the wheelleg for staying below target crawling height (0.22m).
